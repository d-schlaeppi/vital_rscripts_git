### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###
### ### ### ### BEH MAIN Behaviour Analysis - Trophallaxis  ### ### ###
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ###

#### READ ME ####
# THIS VERSION IS FORT 0.8.1 COMPATIBLE 

# Original script created by Adriano Wanderlingh, Nathalie Stroeymeyt and Tom Richardson, with contributions by Enrico Gavagnign
# This version was adjusted by Daniel Schläppi and Nathalie Stroeymeyt to work for trophallactic interactions 
...
USER                       <- "Daniel" # "Nathalie"#"Adriano"
training_set               <- "DS"     # "both" ##Adriano, Vasudha or both
BEH                        <- "T"      # For trophallaxis call T, for Grooming Call "G", for future behaviours other options could be "B" for biting, "A" for aggression, "P" for poison spraying... 
FRAME_RATE                 <- 6        # 8, You might want to call 8 for most purposes, but the trophy data was recorded in 6 frames
...
ITER                       <- 1        # Iteration number for saving output files (+ 1 for every iteration you save) ### at some point it might be an idea to automatically go up +1 for every new iteration. 
...
#} else if (BEH == "P") { # continue the if statement for other types of behaviours
#  interactions_of_interest <- list(c("head", "antenna"))
#} else if (BEH == "B") {
#  interactions_of_interest <- list(c("head", "head"))
} else {
  interactions_of_interest <- NULL 
  print("interactions of interest not defined yet")
}
...
# REP_PER     : each of the 4 blocks analysed, crossing the REPLICATE and PERIOD

# -> the above might need to be adjusted to fit any new users synthax
...
{
# supporting files
WORKDIR <- "/media/gw20248/gismo_hd2/vital/fc2/behaviour_inferral"  # "/media/bzniks/Seagate\ Portable\ Drive/ADRIANO/Ants_behaviour_analysis_EXTRAPOLATED"  
# extrapolated tracking data and myrmidon files
DATADIR <- "/media/gw20248/gismo_hd2/vital/fc2" # "/media/bzniks/Seagate\ Portable\ Drive/ADRIANO/EXPERIMENT_DATA_EXTRAPOLATED"
...
suppressMessages(source(paste(SCRIPTDIR,"BEH_libraries.R",sep="/"))) # I am currently on this line of the code. 
