...
Get_metadata <- function() {
  ...
  for (ant in exp.Ants){
    for (id in ant$identifications){
      metadata <- rbind(metadata,data.frame(REP_treat        = REP_treat,
                                            size_treat       = unlist(strsplit(REP_treat,split="_"))[1],
                                            status           = NA,
                                            antID            = ant$ID,
  ...
  metadata$status[metadata$size_treat=="DF"] <- "Day Pathogen"
  metadata$status[metadata$size_treat=="DS"] <- "Day Sham"
  metadata$status[metadata$size_treat=="NF"] <- "Night Pathogen"
  metadata$status[metadata$size_treat=="NS"] <- "Night Sham" 
...
Metadata_Exp1 <- file.path(DATADIR,"Metadata_Exp1_2022_2023-06-27.txt") 
