...
myrmidon_file1 <- paste("/media/ll16598/SeagateDesktopDrive/SICC_DATA/14S_IV10_180522_POL/EXP_LLEXPP14S_IV10_180522_POL_MAIN_KEY_DEATHS_AutoOriented.myrmidon",sep='')
Metadata_exp <- fmExperimentOpen(myrmidon_file1)

#FOR BEAD CHALLENGED COLONIES
BLUE_ <- paste(dir_chall, "14S_NB.myrmidon",sep='') #Sham SI IDs
BLUE <- fmExperimentOpen(BLUE_)
YELLOW_ <- paste(dir_chall, "14S_LY.myrmidon",sep='') #Sham SI IDs
...
#READ SI FILES
SIBB_ <- paste(dir_SI, "14S_SIBB.myrmidon",sep='') #Beauveria SI IDs
SIBB <- fmExperimentOpen(SIBB_)
SIKVL_ <- paste(dir_SI, "14S_SIKVL.myrmidon",sep='') #Metarhizium SI IDs
SIKVL <- fmExperimentOpen(SIKVL_)
SISH_ <- paste(dir_SI, "14S_SISH.myrmidon",sep='') #Sham SI IDs
