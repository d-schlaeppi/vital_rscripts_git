...
merge_trajectory_segments <- function(positions,IF_frames){
  ...
  for (antID in sort(unique (positions$trajectories_summary$antID))){
    ...
    #   next} #if i do this next no positions output
    if (length(ant_indices)==1){ # LS: this "if statement" should not be commented out! -> otherwise ants with only one trajectory the time will be NA
      ...
      #LL Identify missing columns
      missing_cols <- setdiff(colnames(new_positions[["trajectories_summary"]]), colnames(positions$trajectories_summary[ant_indices,]))
      # Copy the dataframe
      positions_copy <- positions
      # Add the missing column(s)
      for (col in missing_cols) {
        positions_copy$trajectories_summary[,col] <- NA
      }
      new_positions[["trajectories_summary"]] <- rbind(new_positions[["trajectories_summary"]], positions_copy$trajectories_summary[ant_indices,])

      # Then use rbind
      # LL quickfix
      # if (ncol(positions$trajectories_summary[ant_indices,])<4){
      #   next
      # }
...
#DEBUG
#e = e; start = time_start; end = time_stop; maximumGap = MAX_INTERACTION_GAP; computeZones=TRUE;showProgress = FALSE
