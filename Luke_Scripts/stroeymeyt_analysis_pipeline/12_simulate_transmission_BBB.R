...
if (!grepl("survival",data_path)){
  seed_files <- c("treated_worker_list_BBB.txt","random_workers.txt","foragers.txt","nurses.txt", 'worker_list_SIBB.txt','worker_list_SIKVL.txt','worker_list_SISH.txt')
}else{
  seed_files <- c("treated_worker_list_BBB.txt")
}
...
for (seed_file in seed_files ){
  ...
  if (seed_file=="treated_worker_list_BBB.txt"){
    if (!grepl("survival",data_path)){
      outputfolder    <- paste(data_path,"/transmission_simulations/pre_vs_post_treatment/experimentally_exposed_seeds_BBB",sep="")
      interac_folders <- "observed"
      reorder         <- T
    }else{
      outputfolder    <- paste(data_path,"/transmission_simulations/post_treatment/experimentally_exposed_seeds_BBB",sep="")
      interac_folders <- "observed"
      reorder         <- F
    }
  }else{
  ...
  for (interac_folder in interac_folders){
    ...
    if (!file.exists(paste(outputfolder,"/individual_simulation_results_BBB",interac_folder,".txt",sep=""))){
      ...
      if (seed_file!="treated_worker_list_BBB.txt"){interac_list <- interac_list[grepl("PreTreatment",interac_list)]}
      for (interac in interac_list){
        print(interac)
        ####get colony info
...
# LL error
      ...
      write.table(summary_collective,paste(outputfolder,"/collective_simulation_results_BBB",interac_folder,".txt",sep=""),col.names=T,row.names=F,quote=F,append=F)
      write.table(summary_individual,paste(outputfolder,"/individual_simulation_results_BBB",interac_folder,".txt",sep=""),col.names=T,row.names=F,quote=F,append=F)
  ...
  if (grepl("random_vs_observed",outputfolder )){
    ...
    write.table(all_indiv_results,file=paste(outputfolder,"/summarised_individual_results_BBB.dat",sep=""),append=F,quote=F,row.names=F,col.names=T)
