...
#   interaction_table$colony<-str_pad(interaction_table$colony, width = desired_length, pad = "0")
#   write.table(interaction_table, file = file.path(interac), append = F, col.names = T, row.names = F, quote = F, sep = "\t")
# }
#### arguments 
N_SIM  <- 500
if (!grepl("survival",data_path)){
  seed_files <- c("treated_worker_list_KVLY.txt","random_workers.txt","foragers.txt","nurses.txt", 'worker_list_SIBB.txt','worker_list_SIKVL.txt','worker_list_SISH.txt')
}else{
  seed_files <- c("treated_worker_list_KVLY.txt")
}
...
for (seed_file in seed_files ){
  ...
  if (seed_file=="treated_worker_list_KVLY.txt"){
    if (!grepl("survival",data_path)){
      outputfolder    <- paste(data_path,"/transmission_simulations/pre_vs_post_treatment/experimentally_exposed_seeds_KVLY",sep="")
      interac_folders <- "observed"
      reorder         <- T
    }else{
      outputfolder    <- paste(data_path,"/transmission_simulations/post_treatment/experimentally_exposed_seeds_KVLY",sep="")
      interac_folders <- "observed"
      reorder         <- F
    }
  }else{
  ...
  for (interac_folder in interac_folders){
    ...
    if (!file.exists(paste(outputfolder,"/individual_simulation_results_KVLY",interac_folder,".txt",sep=""))){
      summary_collective <- NULL
      summary_individual <- NULL

      interac_list <- input_files[grepl(interac_folder,input_files)]
      interac_list <- unique(interac_list)
      
      if (seed_file!="treated_worker_list_KVLY.txt"){interac_list <- interac_list[grepl("PreTreatment",interac_list)]}
      for (interac in interac_list){
        print(interac)
        ####get colony info
        ...
        # LL
         input_vector <- strsplit(basename(root_name), "_")
        ...
        #tag        <- read.table(tagfile)$tag; names(tag)[names(tag)=="#tag"] <- "tag"; tag <- tag[which(tag$tag!="#tag"),]
        tag <- read.tag(tag_list) #AW
        #maybe take out
        tag$REP_treat <- str_pad(strsplit(basename(tag$REP_treat), "_")[[1]][2], width=desired_length, pad='0')
        #tag<-read.table(tagfile, header = TRUE, sep = "\t")
        if (!grepl("survival",data_path)){
        ...
        skewness_wrapper <- function(x) {
          return(e1071::skewness(x, na.rm = TRUE))
        }
        # LL skewness wrapper
        skewness <- aggregate(na.action="na.pass", final_load ~ sim_number, 
                                   data = simulations[which(simulations$status != "treated"),], 
                                   FUN = skewness_wrapper)
        ...
        # LL removed na.rm=T
        individual_level         <- aggregate(na.action="na.pass", cbind(final_load,contaminated,high_level,low_level) ~ tag,FUN=mean,data=simulations)
...
# LL error
      ...
      write.table(summary_collective,paste(outputfolder,"/collective_simulation_results_KVLY",interac_folder,".txt",sep=""),col.names=T,row.names=F,quote=F,append=F)
      write.table(summary_individual,paste(outputfolder,"/individual_simulation_results_KVLY",interac_folder,".txt",sep=""),col.names=T,row.names=F,quote=F,append=F)
  ...
  if (grepl("random_vs_observed",outputfolder )){
    ...
    write.table(all_indiv_results,file=paste(outputfolder,"/summarised_individual_results_KVLY.dat",sep=""),append=F,quote=F,row.names=F,col.names=T)
