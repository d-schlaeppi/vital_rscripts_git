...
treated_BBB        <- read.table(paste(data_path,"/original_data/treated_worker_list_BBB.txt",sep=""),header=T,stringsAsFactors = F)
treated_KVLY        <- read.table(paste(data_path,"/original_data/treated_worker_list_KVLY.txt",sep=""),header=T,stringsAsFactors = F)
si_KVL        <- read.table(paste(data_path,"/original_data/worker_list_SIKVL.txt",sep=""),header=T,stringsAsFactors = F)
si_BB        <- read.table(paste(data_path,"/original_data/worker_list_SIBB.txt",sep=""),header=T,stringsAsFactors = F)
si_SH        <- read.table(paste(data_path,"/original_data/worker_list_SISH.txt",sep=""),header=T,stringsAsFactors = F)

task_groups    <- read.table(paste(data_path,"original_data/task_groups.txt",sep="/"),header=T,stringsAsFactors = F)
desired_length=3
task_groups$colony<-str_pad(task_groups$colony, width = desired_length, pad = "0")
...
#tag_list <- list.files(tag_list, full.names = TRUE)
####get Queen tag #####
#queenid_list <- 
