...
dir_SI <- "/media/ll16598/One Touch/LUKE/SI_ID/"
dir_chall <- "/media/ll16598/One Touch/SICC_METADATA/CHALL_ID/"

myrmidon_file1 <- paste(dir_data,"1P_V_020222_ALL/LLEXPP1P_IV4_020222_KEY_DEATHS_AutoOriented.myrmidon",sep='')
Metadata_exp <- fmExperimentOpen(myrmidon_file1)

#FORPATHOGEN CHALLENGED COLONIES
CHALL_KVL_ <- paste(dir_chall, "1P_KVL.myrmidon",sep='') #Sham SI IDs
CHALL_KVL <- fmExperimentOpen(CHALL_KVL_)
CHALL_BB_ <- paste(dir_chall, "1P_BB.myrmidon",sep='') #Sham SI IDs
...
write.table(YELLOW_CHALL_LIST, file = paste0(dir_chall, "/YELLOW_CHALL_list",basename(YELLOW_), ".txt", sep=""), append = FALSE,
            row.names = FALSE, col.names = TRUE,quote = FALSE)
#READ SI FILES
SIBB_ <- paste(dir_SI, "LLEXPP1P_020222_ALL/SIBB_LLEXPP1P_IV4_020222.myrmidon",sep='') #Beauveria SI IDs
SIBB <- fmExperimentOpen(SIBB_)
SIKVL_ <- paste(dir_SI, "LLEXPP1P_020222_ALL/SIKVL_LLEXPP1P_IV4_020222",sep='') #Metarhizium SI IDs
SIKVL <- fmExperimentOpen(SIKVL_)
SISH_ <- paste(dir_SI, "LLEXPP1P_020222_ALL/SISHAM_LLEXPP1P_IV4_020222",sep='') #Sham SI IDs
