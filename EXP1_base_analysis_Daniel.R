...
USER <- "Nath_office"  # Replace with the desired USER option: Nath_office, 2A13_Office_Adriano, 2A13_Office_Daniel, AEL-laptop
...
SCRIPTDIR <- paste("/media/",usr, hd, "/vital/scripts/",sep="")
...
# }
#   
# SAVEDIR <- paste("/media/",usr,"/LS_2/circadian_rhythm_2022_experiment/EXP1_base_analysis/EXP_summary_data",sep="")
...
# SCRIPTDIR <- paste("/media",usr,"LS_2/circadian_rhythm_2022_experiment/EXP1_base_analysis/EXP1_analysis_scripts",sep="/") # "/home/cf19810/Documents/scriptsR/EXP1_base_analysis/EXP1_analysis scripts"
...
# metadata_colonies_file <- paste(WORKDIR,"EXP1_base_analysis", "metadata_colonies.csv", sep = "/")
...
get_body_lengths <- function(e,all_body_lengths,REPLICATE) {
  ###check how many spaces there are
  space_list <- e$spaces
  body_lengths <- NULL
  for (space_ID in 1:length(space_list)){
    body_lengths <- rbind(body_lengths,data.frame(space_ID        = space_list[[space_ID]]$ID,
                                                  tracking_system = space_list[[space_ID]]$name,
                                                  body_length     = all_body_lengths[which(all_body_lengths$colony_id==REPLICATE),"mean_ant_lenght_px"],
                                                  body_length_px  = all_body_lengths[which(all_body_lengths$colony_id==REPLICATE),"mean_ant_lenght_px"],
                                                  body_length_mm  = all_body_lengths[which(all_body_lengths$colony_id==REPLICATE),"mean_ant_lenght_mm"]
...
# DS:
Time_dictionary <- data.frame(time_hours = c(-24,0), time_of_day = c(11,11)) # time_hours= time since exposure (negatives pre-exposire, positive post exposure), time_of_day=time hour slot of the day
Time_dictionary$period <- ifelse(Time_dictionary$time_hours<0, "pre", "post")
...
for (REP.file in files_list) {
  REP_colony <- unlist(strsplit(REP.file,split="_"))[grepl("c",unlist(strsplit(REP.file,split="_")))]
  #REP.n <- 14    #temp
  ...
  #for (REP.FILES in REP.filefolder) {
    ...
    ### get body length
    body_lengths <- get_body_lengths(e, colony_metadata, REP_colony)

    # base file info
    # LS:
    REP_name <- unlist(strsplit(REP.file,split="/"))[length(unlist(strsplit(REP.file,split="/")))]
    REP_treatment <- colony_metadata[which(colony_metadata$colony_id==REP_colony),"treatment_simple"]
    REP_ts <-colony_metadata[which(colony_metadata$colony_id==REP_colony),"tracking_system_main"]
...
#       } # REP LOOP
    ...
    for (PERIOD in c("pre","post")) {
      ...
      treatment_code    <- REP_treatment
      
      # Select the full PERIOD (24h)
      cat(paste("#######","period:", PERIOD, sep= " "))
      
      treatment_start <-  as.POSIXct(metadata_colonies[    which   (   metadata_colonies$colony_id == REP_colony   )     ,   "time_treatment_start"     ], format="%Y-%m-%dT%H:%M:%OSZ",  origin="1970-01-01", tz="GMT" )
      ...
      time_end_GMT <-  time_start_GMT + 3*3600
    ...
    # if(!file.exists(INTERACTIONS_FULL)){  #TEMP
      ...
      Interactions <- cbind(data.frame(
        ...
        treatment_detailed=metadata_colonies[which(metadata_colonies$colony_id==REP_colony),"treatment"],
     ...
     # } #TEMP
      ...
      if (RUN_SPACEUSE) {
        ...
        for (TIME_HOURS in Time_dictionary_PERIOD$time_hours) { ## increments by 3 hours for duration of experiment
          ...
          SpaceUsage <- dplyr::left_join(SpaceUsage, metadata[which(metadata$colony_id==REP_colony),c("status_ant", "antID","IsAlive")], by = "antID") # Apply left_join dplyr function
          ...
          SpaceUsage <- cbind(data.frame(
            colony = colony,
            colony_size = unique(metadata[which(metadata$colony_id==REP_colony),"colony_size"]),
