...
#### R Tips I just learned ####
# command shift m  -> will make a pipe %>% 
# option -   -> will insert an assignment operator <- 
...
dat <- read.table("fc1.txt", header = TRUE)
dat$line_nr <- c(1:32) # add a row number
sem <- function(x){sd(x,na.rm=T)/sqrt(length(na.omit(x)))} #defining the standard error function

#### Explore data ####
head(dat)

group_by(dat, experimental_id)  %>%
  summarise(
    count = n()
  )

# create data frame (tibble) containing relevant variables and means 
...
ggplot(df) + 
  ...
  labs(y = "average number of ants", x = "food source") + 
...
boxplot(df$right_mean, df$left_mean) # there seems to be no effect of side (independant of virus or not)
...
boxplot(df$virus_mean, df$control_mean) # there might be an effect of virus on the mean number of ants
...
boxplot(df$virus_mean ~ df$first_side) # choice between left and right (independant of food source) seems unbiased
...
prop.test(10, 32, alternative = "two.sided")
binom.test(10, 32)
...
# based on the dataframe created afterwareds: 
boxplot(df_model$mean_nr_ants ~ df_model$first_discovered*df_model$treatment_model)
boxplot(df_model$mean_nr_ants ~ df_model$treatment_model*df_model$first_discovered)
...
boxplot(number_of_ants2 ~ times, data = plot_frame2)
boxplot(number_of_ants2 ~ times*treatment, data = plot_frame2)
