...
for (time_origin in c("same","shifted")){
  ...
  hist(random_data_Diff$Delta_Nb_ants,col=alpha("grey",0.5),xlim=c(xmin,xmax),main=paste("Observed vs Expected, Nb Ants Virus minus Healthy,",capitalize(time_origin),"T0"))
...
#### new idea : linear model with time all time points included instead of means ?

head(dynamic_dat_same_t0_Nb)
str(dynamic_dat_same_t0_Nb)
dynamic_dat_same_t0_Nb$time <- as.factor(dynamic_dat_same_t0_Nb$time)
dynamic_dat_same_t0_Nb$feeding_session <- as.factor(dynamic_dat_same_t0_Nb$feeding_session)

model <- glmer(Nb_ants ~ status*time + feeding_session + (1|colony_id) + (1|first_source_discovered), data=dynamic_dat_same_t0_Nb, family = "poisson")
Anova(model)

# or with feedign session as random factor
model <- glmer(Nb_ants ~ status*time + (1|colony_id) + (1|first_source_discovered) + (1|feeding_session), data=dynamic_dat_same_t0_Nb, family = "poisson")
Anova(model)
