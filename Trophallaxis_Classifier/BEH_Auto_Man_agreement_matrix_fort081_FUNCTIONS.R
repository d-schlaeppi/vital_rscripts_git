...
auto_manual_agreement <- function(summary_AUTO , summary_MANUAL, list_IF_Frames, list_replicate_full_ant_list, list_replicate_focal_list ){
  ...
  for (REPLICATE in gsub("replicate_list_","",names(list_replicate_full_ant_list))){
    for (PERIOD in unique(gsub(paste("IF_frames_",REPLICATE,"_",sep=""),"",names(list_IF_Frames)[which(grepl(REPLICATE,names(list_IF_Frames)))]))){
      ...
      if (length(which(summary_AUTO   $REPLICATE==REPLICATE & summary_AUTO   $PERIOD==PERIOD))>0){
