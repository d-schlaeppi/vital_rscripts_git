...
extract_trajectories <- function (e, start ,end , maximumGap ,computeZones=TRUE,showProgress = FALSE, IF_frames){
  ...
  while (!extraction_complete){
    ...
    if (as.numeric(as.POSIXct(capture.output(print(end)), format = "%Y-%m-%dT%H:%M:%OSZ",  origin="1970-01-01", tz="GMT" ) ) -   as.numeric(as.POSIXct(capture.output(print(start)), format = "%Y-%m-%dT%H:%M:%OSZ",  origin="1970-01-01", tz="GMT" ) ) > time_chunk){
      ###if superior, define new end time as equal to start + 12 hours
      frame_end <- IF_frames[6 + match.closest(x = IF_frames[which(IF_frames$frame_num==frame_start),"time"]+time_chunk,table = as.numeric(IF_frames$time)),"frame_num"] ###add six to jump gaps in tracking where they are bigger than time_chunk
    ...
    if (!extraction_complete){
      ###first, check that more than 5 frames were extracted or you will be stuck in an infinite loop!
      if (max(unlist(lapply(positions$trajectories,function(x)x[nrow(x),"time"])))>(5/FRAME_RATE)){
      ...
      }else{###otherwise, tracking was interrupted - in that case use the end of the previous period as a start for the new one
        start <- fmTimeCreate(offset=IF_frames[which(IF_frames$frame_num==frame_end - 5 ) ,"time"   ]       ) ###extract from a few frames before to make sure no data is lost, as rounding times leads to uncertainties
      }
