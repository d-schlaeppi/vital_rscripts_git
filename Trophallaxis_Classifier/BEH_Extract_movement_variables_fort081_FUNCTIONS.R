extraction_loop <- function(chunk,extract_movement_variables=T, selected_variables = NULL,all_body_lengths=NULL,focal="all",focal_list=NULL){
  ...
  for (REPLICATE in unique(time_window_ori$REPLICATE)) 
  {
    ...
    body_lengths <- get_body_lengths(e,all_body_lengths,REPLICATE)
    ...
    for (PERIOD in unique(time_window_ori[which(time_window_ori$REPLICATE==REPLICATE),       "PERIOD"]))
    {
      ...
      if (nrow(annotations[which(annotations$Behaviour==BEH&annotations$REPLICATE==REPLICATE&annotations$PERIOD==PERIOD),])>0){
        if (extract_movement_variables){
          ...
          if (!"ant1"%in%names(annot_per_rep)){
            annot_per_rep$ant1 <- annot_per_rep$Actor; annot_per_rep$ant2 <- annot_per_rep$Receiver;
          }
          annot_per_rep$pair <- apply(annot_per_rep[,c("ant1","ant2")],1,function(x){paste(sort(as.numeric(x)),collapse = "_") })
      ...
      if (nrow(interacts_AUTO_REP_PER)>0){
        if (exclude_queen){
          interacts_AUTO_REP_PER <- interacts_AUTO_REP_PER[which(interacts_AUTO_REP_PER$ant1!=queen_id&interacts_AUTO_REP_PER$ant2!=queen_id),]
        }
