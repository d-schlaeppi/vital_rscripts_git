...
index            <- as.numeric  (commandArgs()[as.numeric(length(commandArgs())   )])
...
DATADIR                    <- "/user/work/bzniks/EXPERIMENT_DATA_EXTRAPOLATED"
SCRIPTDIR                  <- "/user/home/bzniks/code/ScriptsR_FINAL"
BODYLENGTH_FILE            <- paste(DATADIR,"/Mean_ant_length_per_TrackingSystem.txt",sep="/")
MachineLearningOutcome_DIR <- file.path(DATADIR, "MachineLearning_outcomes_FINAL")
MYRMIDON_FILE              <- paste(SCRIPTDIR,"/argument_list_grooming_extraction.txt",sep="/")
...
myrmidon_files   <-read.table(MYRMIDON_FILE,header=T,stringsAsFactors = F,sep=",")
...
###############################################################################
myrmidon_file <- myrmidon_files[index,"myrmidon_file"]
setwd(DATADIR)
to_keep <- c(ls(),"to_keep")
  ...
  output_name <- file.path(DATADIR,"Adriano_extracted_groomings_final",paste("inferred_groomings_",unlist(strsplit(REPLICATE,split="\\/"))[which(grepl("BS|BP|SS|SP",unlist(strsplit(REPLICATE,split="\\/"))))],"_",PERIOD,".txt",sep=""))
  output_name_with_nongroom <- file.path(DATADIR,"Adriano_extracted_groomings_final",paste("inferred_groomings_and_nongroom_",unlist(strsplit(REPLICATE,split="\\/"))[which(grepl("BS|BP|SS|SP",unlist(strsplit(REPLICATE,split="\\/"))))],"_",PERIOD,".txt",sep=""))
