...
for (seed_file in seed_files ){
  ...
  if (seed_file=="treated_workers.txt" || TRUE){ #AW 26Jul2023: added || TRUE to also apply this block to other seed files
    if (!grepl("survival",data_path)){
      outputfolder    <- paste(data_path,"/transmission_simulations/pre_vs_post_treatment/",gsub("\\.txt","_seeds",seed_file),sep="") #AW 26Jul2023: modified to include seed_file in outputfolder
      interac_folders <- "observed"
      reorder         <- T
    }else{
      outputfolder    <- paste(data_path,"/transmission_simulations/post_treatment/",gsub("\\.txt","_seeds",seed_file),sep="") #AW 26Jul2023: modified to include seed_file in outputfolder
      interac_folders <- "observed"
      reorder         <- F
    }
  }else{
