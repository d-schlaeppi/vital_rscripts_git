...
setwd(input_path)  
input_files        <- paste(input_path,"/",list.files(recursive=T,pattern="interactions"),sep="")        # questions: done for all interaction networks random and observed? 
...
for (seed_file in seed_files ){ # seed_file <- "treated_workers"
  ...
  if (seed_file=="treated_workers.txt"){
    ...
    interac_folders <- c("observed",1:100) # LS: 1:5 is just for testing! normally 1:100
  }
  ...
  for (interac_folder in interac_folders){ # interac_folder <- "observed"
    if (interac_folder!="observed"){interac_folder <- paste("random_",paste(rep(0,3-nchar(interac_folder)),collapse=""),interac_folder,sep="")} ##### I neeed to adjust this line or in the previous script make sure that the randomized files will be put into the same folder or alternatively change this script here 
    if (!file.exists(paste(outputfolder,"/individual_simulation_results_",interac_folder,".txt",sep=""))){
      ...
      for (interac in interac_list){
        ...
        root_name          <- gsub("_interactions.txt","",unlist(strsplit(interac,split="/"))[grepl("interactions",unlist(strsplit(interac,split="/")))]) # LS: replace grepl("colony", ...) with grepl("interactions")
        colony             <- unlist(strsplit(root_name,split="_"))[1] # LS
        period_detail      <- unlist(strsplit(root_name,split="_"))[4] # LS 
        treatment_circadian<-  unlist(strsplit(unlist(strsplit(root_name,split="_"))[2],split="\\."))[2] # LS
        if (period_detail=="pre1"|period_detail=="post"){ # LS
        ...
        # LS: Warning? should be fine -> first element will be "observed" rest "random_001" and so forth...
        ...
        for (sim in unique(simulations$sim_number)){  # LS: warnings but only that elements produced in try() do not exist in some cases
        ...
        for (ant in alive){
          if (all(simulations[which(simulations$tag==ant),"contaminated"]==1)){
            ...
            individual_level[which(individual_level$tag==ant),"transmission_latency"] <-  mean_data["rmean"] # LS: column name has changed to "rmean"
            
            model                                                                  <- coxph(Surv(rank,contaminated)~1,data=simulations[which(simulations$tag==ant),])
            mean_data                                                              <- summary(survfit(model),rmean="common")$table
            individual_level[which(individual_level$tag==ant),"transmission_rank"] <-  mean_data["rmean"] # LS: column name has changed to "rmean"
        ...
        individual_level["period_detail"]      <- period_detail       # LS
        individual_level["period_circadian"]      <- period_circadian # LS
        individual_level["time_hours"]  <- interaction_table[1,"time_hours"]
        individual_level["time_of_day"] <- interaction_table[1,"time_of_day"]
        
        individual_level       <- individual_level[c("colony","colony_size","treatment","tag","antid","status","period","period_detail","period_circadian","time_hours","time_of_day","simulated_load","probability_of_transmission","probability_high_level","probability_low_level","transmission_latency","transmission_rank")] # LS: period_detail & period_circadian
