...
for (input_folder in input_folders){ # input_folder <- "random"
  ...
  # summary_interactions_trophy <- NULL #### SE IF THIS IS NEEDED  
  summary_pairs        <- NULL
  ...
  for (network_file in network_files){ # network_file <- network_files[1]
    ...
    summary_interactions <- rbind(summary_interactions,data.frame(randy=input_folder,colony=colony,colony_size=colony_size,treatment=treatment,period=period,time_hours=time_hours,time_of_day=time_of_day,full_table,stringsAsFactors = F)) # LS: add period_detail & period_circadian; DS: removed LS inputs as not needed due to different experimental design - period_detail=period_detail, period_circadian=period_circadian,
...
# calculations done --> save summary tables
