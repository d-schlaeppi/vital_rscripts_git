...
###HERE IS THE CHANGE FROM healthy to control
...
for (start_time in c("start_experiment", "discovery_first_food", "shifted_discovery")) { # keep shifted for last...# start_time <- "shifted_discovery"
  ...
  model <- lmer(log10(exploitation_rate) ~ food_source + (1|colony_id) + (1|first_source_fed_on), data=summary_dat_duration )
  print(Anova(model))
  pvalue <- round(Anova(model)$`Pr(>Chisq)`,2)
  print(shapiro.test(residuals(model))) # for shifted t0 not significant... 
  
  model <- lmer((exploitation_rate) ~ food_source*first_source_fed_on_binary + (1|colony_id) , data=summary_dat_duration )
  print(shapiro.test(residuals(model))) # for shifted t0 not significant... 
  print(Anova(model))
  pvalue <- round(Anova(model)$`Pr(>Chisq)`,2)
  print(shapiro.test(residuals(model))) # for shifted t0 not significant... 
  ...
  names(summary_dat_duration_first)[names(summary_dat_duration_first)=="exploitation_rate"] <- "exploitation_rate_first_source_discovered"
  summary_dat_duration_second <- summary_dat_duration[which(!summary_dat_duration$first_source_fed_on_binary),c("colony_id","exploitation_rate","first_source_fed_on")]
  names(summary_dat_duration_second)[names(summary_dat_duration_second)=="exploitation_rate"] <- "exploitation_rate_second_source_discovered"
  ...
  cumulative_explotation_over_time$first_source_fed_on_binary <- cumulative_explotation_over_time$food_source==cumulative_explotation_over_time$first_source_fed_on
  cumulative_explotation_over_time_first  <- cumulative_explotation_over_time[which(cumulative_explotation_over_time$first_source_fed_on_binary),c("colony_id","time","cumulated_exploitation_time","exploitation_rate","first_source_fed_on")]
  ...
  cumulative_explotation_over_time_second <- cumulative_explotation_over_time[which(!cumulative_explotation_over_time$first_source_fed_on_binary),c("colony_id","time","cumulated_exploitation_time","exploitation_rate","first_source_fed_on")]
...
if (RUN_ANALYSIS_AND_SIMULATIONS_DURATION) { # RUN_ANALYSIS_AND_SIMULATIONS_DURATION <- TRUE
  ...
  for (start_time in c("shifted_discovery")) { # start_time <- "shifted_discovery" # Loop over different definitions of †0 an other option would be # "start_experiment",  "discovery_first_food", # start_time = "discovery_first_food"
    ...
    for (i in 1 : total_iterations){ ### randomisation loop # i <- 1
      ...
      hypothetic_virus_first    <- sample(colony_list,size=nb_virus,replace = F)
      nb_shared                 <- length(which(hypothetic_virus_first %in%colony_metadata[which(colony_metadata$first_source_fed_on=="virus"),"colony_id"]))
      nb_differing              <- length(which(!hypothetic_virus_first %in%colony_metadata[which(colony_metadata$first_source_fed_on=="virus"),"colony_id"]))
      prop_differing            <- nb_differing/(nb_differing+nb_shared)
      ...
      rand_subset_dur[which(rand_subset_dur$first_source_discovered_RAND!=rand_subset_dur$first_source_fed_on &  rand_subset_dur$food_source=="control"), "food_source_RAND"]    <- "virus" ##~ERROR: change "healthy" to "control"
      ...
      # concatenate
      random_data_dynamic_dur           <- rbind(random_data_dynamic_dur , data.frame(RAND=i, mean_rand_dat_dur,prop_differing=prop_differing))
      mean_random_data_dynamic_dur      <- rbind(mean_random_data_dynamic_dur , data.frame(RAND=i, overall_mean_rand_dat_dur,prop_differing))
      pb$tick()
    }
