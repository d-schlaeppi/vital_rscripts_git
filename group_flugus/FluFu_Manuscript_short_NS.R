...
setwd("C:/Users/Ellie/OneDrive - Nexus365/Documents/Bristol research project/FLUGUS")
getwd()
...
{
null_model          <- coxme ( Surv (time = survival_14, event = censor_14) ~ 1                                 + (1 | petri_dish) , data = antdata2)
flupy_model_f       <- coxme ( Surv (time = survival_14, event = censor_14) ~ 1 + concentration_factor          + (1 | petri_dish)  , data = antdata2)
fungus_model_f      <- coxme ( Surv (time = survival_14, event = censor_14) ~ 1 + concentration_factor + fungus + (1 | petri_dish) , data = antdata2)
interaction_model_f <- coxme ( Surv (time = survival_14, event = censor_14) ~ 1 + concentration_factor * fungus + (1 | petri_dish) , data = antdata2)
