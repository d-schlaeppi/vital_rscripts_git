...
# re run capsule cloner to get the 2018 capsules to all my myrmidon files.
...
# Instead of the 3 hour block starting from 11 until 14 use exact starting and stop time. (start time should have already been implemented)
...
# files_list <- list.files(DATADIR, pattern="CapsuleDef2018.myrmidon")                                         # to run once I fixed the mistake on the capsule coloner            
files_list <- list.files(DATADIR, pattern="CapsuleDef00.myrmidon")
...
#### Loop over each colony to calculate basic interactions and space use ####
...
for (REP.file in files_list) {                                                                        
    ...
    for (PERIOD in c("pre","post")) {                                                                                 # for the pre and the post treatment time block 
      ...
      dead_by_REP <- metadata[which(metadata$IsAlive==FALSE & metadata$colony_id==colony_id),"antID"]
