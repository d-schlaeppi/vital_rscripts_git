...
# This is Adrianos & Nathalies script and got adjusted to the data structure and needs of Daniel

# I introduced a forced memory cleaning after every gc() - also inside the trajectory_extraction.R - which helps a lot in freeing memory.
# free memory by installing in terminal:
#   > sudo apt-get install libtcmalloc-minimal4
# then in R:
#   > install.packages("mallinfo", repos = "http://www.rforge.net/")
# and run the following after gc():
#   > mallinfo::malloc.trim(0L)
...
{if (USER == "Nath_office") {
  ...
  usr <- "ael"
} else { # Default value in case of an unrecognized USER option
  usr <- NULL
}
if (!is.null(usr)) { # Print the value of usr if defined, otherwise print the message
  print(usr)
} else {
  print("define new user if necessary")}
}
...
{if (HD == "Nathalie") {
  ...
  if (!is.null(hd)) { # Print the value of usr if defined, otherwise print the message
    print(hd)
  } else {
    print("define new user if necessary")}
}
...
INTDIR <- paste("/media/",usr, hd, "/vital/fc2/Lasius-Bristol_pathogen_experiment/main_experiment/intermediary_analysis_steps",sep="")
BEHDIR <- paste("/media/",usr, hd, "/vital/fc2/Lasius-Bristol_pathogen_experiment/main_experiment/processed_data/individual_behaviour",sep="")
WORKDIR <- paste("/media/",usr, hd, "/vital/fc2",sep="")
DATADIR <- paste(WORKDIR, sep = "/")
SCRIPTDIR <- paste("/media/",usr, hd, "/vital/fc2/Documents/EXP1_base_analysis/EXP1_analysis_scripts", sep="")
SCRIPTDIR_Daniel <- paste("/home/",usr,"/Documents/vital_rscripts_git",sep="") # "/home/cf19810/Documents/scriptsR/EXP1_base_analysis/EXP1_analysis scripts"
BEH_FUNCTIONS <-  paste("/media",usr, hd, "vital/fc2/EXP1_base_analysis/EXP1_analysis_scripts/Behavioural_Inference",sep="/")
...
##metadata
  ...
  source(paste0(directory_scripts,"vital_meta_data.R")) # adjust for this script
